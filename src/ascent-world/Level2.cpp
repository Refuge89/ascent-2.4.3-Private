...
bool ChatHandler::HandleInvisibleCommand(const char *args, WorldSession *m_session)
{
	...
	SystemMessage(m_session, (const char*)msg);
	return true;
}
...
bool ChatHandler::HandleDeleteCommand(const char* args, WorldSession *m_session)
{
	...
	if(unit->m_spawn == 0)
		return false;
	SystemMessage(m_session, "Deleted creature ID %u", unit->spawnid);
...
bool ChatHandler::HandleSaveAllCommand(const char *args, WorldSession *m_session)
{
	...
	char msg[100];
	snprintf(msg, 100, "Saved all %d players in %d msec.", (int)count, int((uint32)now() - stime));
...
bool ChatHandler::HandleKillCommand(const char *args, WorldSession *m_session)
{
	...
	if(target == 0)
	{
		SystemMessage(m_session, "A valid selection is required.");
		return true;
	}
	...
	if(target->IsPlayer())
	{
		...
		plr->KillPlayer();
		SystemMessageToPlr(plr, "%s killed you with a GM command.", m_session->GetPlayer()->GetName());
	}
	else
	{
...
bool ChatHandler::HandleKillByPlrCommand( const char *args , WorldSession *m_session )
{
	...
	if(!plr)
	{
		SystemMessage(m_session, "Player %s is not online or does not exist.", args);
		return true;
	}

	if(plr->isDead())
	{
		SystemMessage(m_session, "Player %s is already dead.", args);
	} else {
		plr->SetUInt32Value(UNIT_FIELD_HEALTH, 0); // Die, insect
		plr->KillPlayer();
		SystemMessageToPlr(plr, "You were killed by %s with a GM command.", m_session->GetPlayer()->GetName());
		SystemMessage(m_session, "Killed player %s.", args);
...
bool ChatHandler::HandleCastSpellCommand(const char* args, WorldSession *m_session)
{
	...
	if(!target)
	{
		SystemMessage(m_session, "Must select a char or creature.");
		return false;
	}
	...
	if(!spellentry)
	{
		SystemMessage(m_session, "Invalid spell id!");
		return false;
	}
	...
	if(!sp)
	{
		SystemMessage(m_session, "Spell failed creation!");
		delete sp;
		return false;
	}

	SystemMessage(m_session, "Casting spell %d on target.", spellid);
...
bool ChatHandler::HandleMonsterSayCommand(const char* args, WorldSession *m_session)
{
	...
	if(!crt)
	{
		SystemMessage(m_session, "Please select a creature or player before using this command.");
		return true;
	}
...
bool ChatHandler::HandleGOSelect(const char *args, WorldSession *m_session)
{
	...
	if( GObj == NULL )
	{
		SystemMessage(m_session, "No inrange GameObject found.");
		return true;
	}

	m_session->GetPlayer()->m_GM_SelectedGO = GObj;

	SystemMessage(m_session, "Selected GameObject [ %s ] which is %.3f meters away from you.",
...
bool ChatHandler::HandleGODelete(const char *args, WorldSession *m_session)
{
  ...
  /*  std::stringstream sstext;
	...
	SystemMessage(m_session, "GameObject successfully deleted from world and database !");
*/
	return true;
}
...
bool ChatHandler::HandleGOInfo(const char *args, WorldSession *m_session)
{
	...
	if( !GOInfo )
	{
		SystemMessage(m_session, "This GameObject doesn't have template, you won't be able to get some informations nor to spawn a GO with this entry.");
		return true;
	}
	else
	{
	SystemMessage(m_session,"Informations:");
	SystemMessage(m_session,"Entry:%s %d",MSG_COLOR_LIGHTBLUE, GObj->GetEntry());
	SystemMessage(m_session,"Model:%s %d",MSG_COLOR_LIGHTBLUE, GObj->GetUInt32Value(GAMEOBJECT_DISPLAYID));
	SystemMessage(m_session,"State:%s %d",MSG_COLOR_LIGHTBLUE, GObj->GetUInt32Value(GAMEOBJECT_STATE));
	SystemMessage(m_session,"Flags:%s %d",MSG_COLOR_LIGHTBLUE, GObj->GetUInt32Value(GAMEOBJECT_FLAGS));
	SystemMessage(m_session,"DynFlags:%s %d",MSG_COLOR_LIGHTBLUE, GObj->GetUInt32Value(GAMEOBJECT_DYN_FLAGS));
	SystemMessage(m_session,"Faction:%s %d",MSG_COLOR_LIGHTBLUE, GObj->GetUInt32Value(GAMEOBJECT_FACTION));
	SystemMessage(m_session,"TypeID:%s %d",MSG_COLOR_LIGHTBLUE, GObj->GetUInt32Value(GAMEOBJECT_TYPE_ID));
	SystemMessage(m_session,"Type:%s %s", MSG_COLOR_LIGHTBLUE, name.c_str());
	SystemMessage(m_session," "); //force whitespace line :/
	SystemMessage(m_session,"Distance:%s %d metres",MSG_COLOR_LIGHTBLUE, uint32(GObj->CalcDistance((Object*)m_session->GetPlayer()))); //brackets much :D
	SystemMessage(m_session,"Name:%s %s",MSG_COLOR_LIGHTBLUE, GOInfo->Name);
	SystemMessage(m_session,"Size:%s %f",MSG_COLOR_LIGHTBLUE, GObj->GetFloatValue(OBJECT_FIELD_SCALE_X));
	}
...
bool ChatHandler::HandleGOEnable(const char *args, WorldSession *m_session)
{
	...
	SystemMessage(m_session, "Gameobject activate/deactivated.");
	return true;
}
...
bool ChatHandler::HandleGOActivate(const char* args, WorldSession *m_session)
{
	...
	SystemMessage(m_session, "Gameobject opened/closed.");
	return true;
}
...
bool ChatHandler::HandleGOScale(const char* args, WorldSession* m_session)
{
	...
	if(!args)
	{
		SystemMessage(m_session, "Invalid syntax. Should be .gobject scale 1.0");
		return false;
	}
	float scale = (float)atof(args);
	if(!scale) scale = 1;
	go->SetFloatValue(OBJECT_FIELD_SCALE_X, scale);
	SystemMessage(m_session, "Set scale to %.3f", scale);
	return true;
}
...
bool ChatHandler::HandleReviveStringcommand(const char* args, WorldSession* m_session)
{
	...
	if(!plr)
	{
		SystemMessage(m_session, "Could not find player %s.", args);
		return true;
	}
	...
	if(plr->isDead())
	{
		...
		SystemMessage(m_session, "Revived player %s.", args);
	} else {
...
bool ChatHandler::HandleMountCommand(const char *args, WorldSession *m_session)
{
	if(!args)
	{
		SystemMessage(m_session, "No model specified");
		return true;
	}
	...
	if(!m_target)
	{
		SystemMessage(m_session, "No target found.");
		return true;
	}
	
	if(m_target->GetUInt32Value(UNIT_FIELD_MOUNTDISPLAYID) != 0)
	{
		SystemMessage(m_session, "Target is already mounted.");
		return true;
	}
	...
	SystemMessage(m_session, "Now mounted with model %d.", modelid);
	return true;
}
...
bool ChatHandler::HandleAddAIAgentCommand(const char* args, WorldSession *m_session)
{
	...
	if(!target)
	{
		SystemMessage(m_session, "You have to select a Creature!");
		return false;
	}
...
bool ChatHandler::HandleGOAnimProgress(const char * args, WorldSession * m_session)
{
	...
	SystemMessage(m_session, "Set ANIMPROGRESS to %u", ap);
	return true;
}
...
bool ChatHandler::HandleGOExport(const char * args, WorldSession * m_session)
{
	...
	SystemMessage(m_session, "Go saved to: %s", name.str().c_str());*/
