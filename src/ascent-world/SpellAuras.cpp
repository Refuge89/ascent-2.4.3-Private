...
void Aura::EventPeriodicDamage(uint32 amount)
{
	...
	if(m_spellProto->NameHash == SPELL_HASH_SEED_OF_CORRUPTION)
	{
		...
		{
			c->CastSpellAoF(m_target->GetPositionX(), m_target->GetPositionY(), m_target->GetPositionZ(), dbcStorage.Lookup(27285), true);
			Remove();
		}
	}
...
void Aura::SpellAuraModStun(bool apply)
{
	...
	{
		if(m_caster->GetUInt32Value(UNIT_FIELD_CHANNEL_OBJECT))
		{
			if(m_caster->GetCurrentSpell() != NULL)
				m_caster->CancelSpell(m_caster->GetCurrentSpell())
		}
			...
			if(m_spellProto->NameHash == SPELL_HASH_DRAIN_LIFE)
			{
				...
				if(HasSoulSiphon)
				{
					...
					for(int i = MAX_POSITIVE_AURAS; i <= MAX_AURAS; i++)
					{
						...
						{
							if(objmgr.GetSpellSkill(m_target->m_auras[i]->GetSpellId()) == SKILL_AFFLICTION)
							{
								AffCount++;
								TotalSSPer += PerAff;
								if(AffCount >= MaxAff)
