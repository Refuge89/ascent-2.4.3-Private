...
void WorldSession::HandleCharCreateOpcode( WorldPacket & recv_data )
{
	...
	//WorldSession *m_session = pNewChar->GetSession();
...
#ifdef WIN32
	_player->BroadcastMessage("Server: %sKamEMU %s r%u/%s-Win-%s %s(www.kamical.org)", MSG_COLOR_WHITE, BUILD_TAG,
		BUILD_REVISION, CONFIG, ARCH, MSG_COLOR_LIGHTBLUE);		
#else
	_player->BroadcastMessage("Server: %sKamEMU %s r%u/%s-%s %s(www.kamical.org)", MSG_COLOR_WHITE, BUILD_TAG,
