...
//	ChatCommand *table = getCommandTable();
	...
	{
		SystemMessage(m_session, "Sorry, no help was found for this command, or that command does not exist.");
	}
...
bool ChatHandler::HandleStartCommand(const char* args, WorldSession *m_session)
{
	...
	else if (m_plyr && args && strlen(args) > 2)
	{
		...
		{
			SystemMessage(m_session, "Invalid start location! Valid locations are: human, dwarf, gnome, nightelf, draenei, orc, troll, tauren, undead, bloodelf");
			return true;
		}
	}
	else
	...
	if(info == NULL)
	{
		SystemMessage(m_session, "Internal error: Could not find create info.");
		return false;
	}


	SystemMessage(m_session, "Telporting %s to %s starting location.", m_plyr->GetName(), race.c_str());
...
bool ChatHandler::HandleInfoCommand(const char* args, WorldSession *m_session)
{
	...
	SystemMessage(m_session, "Server Revision: |r%sKamEmu r%u/%s-%s-%s %s(www.kamical.org)", MSG_COLOR_WHITE,
		BUILD_REVISION, CONFIG, PLATFORM_TEXT, ARCH, MSG_COLOR_LIGHTBLUE);
	SystemMessage(m_session, "Server Uptime: |r%s", sWorld.GetUptimeString().c_str());
	SystemMessage(m_session, "Current Players: |r%d (%d GMs, %d queued)", clientsNum, gm,  0);
	SystemMessage(m_session, "Active Thread Count: |r%u", ThreadPool.GetActiveThreadCount());
	SystemMessage(m_session, "Free Thread Count: |r%u", ThreadPool.GetFreeThreadCount());
	SystemMessage(m_session, "Average Latency: |r%.3fms", (float)((float)avg / (float)count));
	SystemMessage(m_session, "SQL Query Cache Size (World): |r%u queries delayed", WorldDatabase.GetQueueSize());
	SystemMessage(m_session, "SQL Query Cache Size (Character): |r%u queries delayed", CharacterDatabase.GetQueueSize());
...
{
	SystemMessage(m_session, "Not yet implemented.");
	return true;
}
...
bool ChatHandler::HandleDismountCommand(const char* args, WorldSession *m_session)
{
	...
	{
		SystemMessage(m_session, "Target is not mounted.");
		return true;
	}
	...
	SystemMessage(m_session, "Now unmounted.");
	return true;
}
...
bool ChatHandler::HandleSaveCommand(const char* args, WorldSession *m_session)
{
	...
	if(m_session->GetPlayer()->m_nextSave < 300000 || m_session->CanUseCommand('s')) //5min out of 10 left so 5 min since last save
	{
		...
		SystemMessage(m_session, "Player saved to DB");
	}
	else
	{
		SystemMessage(m_session, "You can only save once every 5 minutes.");
	}
	m_session->SendPacket( &data );
	return true;
}
...
bool ChatHandler::HandleGMListCommand(const char* args, WorldSession *m_session)
{
	...
	for (itr = objmgr._players.begin(); itr != objmgr._players.end(); itr++)
	{
		...
		{
			if(first)
				SystemMessage(m_session, "There are following active GMs on this server:");
...
bool ChatHandler::HandleGmLogCommentCommand( const char *args , WorldSession *m_session )
{
	...
	SystemMessage(m_session, "Added Logcomment: %s",args);
