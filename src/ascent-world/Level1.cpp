...
bool ChatHandler::HandleGMOnCommand(const char* args, WorldSession *m_session)
{
	...
	SystemMessage(m_session, "Setting GM Flag on yourself...");
	if(m_session->GetPlayer()->bGMTagOn)
		SystemMessage(m_session, "GM Flag is already set on. Use !gmoff to disable it.");
	else
	{
		m_session->GetPlayer()->bGMTagOn = true;
		m_session->GetPlayer()->SetFlag(PLAYER_FLAGS, PLAYER_FLAG_GM);	// <GM>
		SystemMessage(m_session, "GM Flag Set. It will appear above your name and in chat messages until you use !gmoff.");
	}
...
bool ChatHandler::HandleGMOffCommand(const char* args, WorldSession *m_session)
{
	...
	//SystemMessage(m_session, "GM Flag Unset.");
	if(sWorld.m_forceGMTag && !m_session->CanUseCommand('z'))
	{
		SystemMessage(m_session, "You are forced to use the GM tag.");
		return false;
	}
	else
	{
	SystemMessage(m_session, "Unsetting GM Flag on yourself.");
	if(!m_session->GetPlayer()->bGMTagOn)
		SystemMessage(m_session, "GM Flag not set. Use !gmon to enable it.");
	else
	{
		m_session->GetPlayer()->bGMTagOn = false;
		m_session->GetPlayer()->RemoveFlag(PLAYER_FLAGS, PLAYER_FLAG_GM);	// <GM>
		SystemMessage(m_session, "GM Flag Removed. <GM> Will no longer show in chat messages or above your name.");
	}
	return true;
	}
}
...
bool ChatHandler::HandleKickCommand(const char* args, WorldSession *m_session)
{
	...
	if(!pname)
	{
		SystemMessage(m_session, "No name specified.");
		return true;
	}
	...
	if (chr)
	{
		...
		SystemMessage(m_session, "Attempting to kick %s from the server for \"%s\".", chr->GetName(), kickreason.c_str());
		...
		{
			SystemMessage(m_session, "You cannot kick %s, as they are a higher level gm than you.", chr->GetName());
			return true;
		}
	...
	{
		SystemMessage(m_session, "Player is not online at the moment.");
		return true;
	}
	return true;
}
...
bool ChatHandler::HandleAddFriendCommand(const char* args, WorldSession *m_session)
{
	...
	if(!fname)
	{
		SystemMessage(m_session, "No friend to add specified.");
		return false;
	}
...
bool ChatHandler::HandleModifySpeedCommand(const char* args, WorldSession *m_session)
{
	...
	{
		SystemMessage(m_session, "Incorrect value. Range is 1..255");
		return true;
	}
	...
	// send message to user
	SystemMessage(m_session, "You set the speed of %s to %2.2f.", chr->GetName(), Speed);
...
bool ChatHandler::HandleLearnSkillCommand(const char *args, WorldSession *m_session)
{
	...
	SystemMessage(m_session, "Adding skill line %d", skill);
...
bool ChatHandler::HandleGetSkillLevelCommand(const char *args, WorldSession *m_session)
{
	...
	{
		SystemMessage(m_session, "Skill: %u does not exists", skill);
		return false;
	}
    ...
    if (!plr->_HasSkillLine(skill))
    {
        SystemMessage(m_session, "Player does not have %s skill.", SkillName);
        return false;
    }
    ...
    SystemMessage(m_session, "Player's %s skill has level: %u maxlevel: %u. (+ %u bonus)", SkillName,max,nobonus, bonus);
	return true;
}
...
bool ChatHandler::HandleGetSkillsInfoCommand(const char *args, WorldSession *m_session)
{
    ...
    SystemMessage(m_session, "Player: %s has skills", plr->GetName() );
    ...
    for (uint32 SkillId = 0; SkillId <= SkillNameManager->maxskill; SkillId++)
    {
        if (plr->_HasSkillLine(SkillId))
        {
            ...
            if (!SkillName)
            {
                SystemMessage(m_session, "Invalid skill: %u", SkillId);
                continue;
            }
            ...
            SystemMessage(m_session, "  %s: Value: %u, MaxValue: %u. (+ %d bonus)", SkillName, nobonus,max, bonus);
        }
    }
...
bool ChatHandler::HandleRemoveSkillCommand(const char *args, WorldSession *m_session)
{
	...
	if(!pSkill)
		...
		skill = atol(pSkill);
	SystemMessage(m_session, "Removing skill line %d", skill);
	...
	{
		SystemMessage(m_session, "Player doesn't have skill line %d", skill);
	}
	return true;
}
...
//	WorldPacket data;
	...
	if(newgold < 0)
	{
		SystemMessage( m_session, "Taking all gold from %s's backpack...", chr->GetName() );
		SystemMessageToPlr(chr, "%s took the all gold from your backpack.", m_session->GetPlayer()->GetName());
		newgold = 0;
	}
	else
	{
		if(total >= 0) {
			...
			SystemMessageToPlr( chr, "%s added %u gold, %u silver, %u copper to your backpack.",
				m_session->GetPlayer()->GetName(),
				gold, silver, copper );
		}
		else
		{
			...
			SystemMessageToPlr( chr, "%s took %u gold, %u silver, %u copper from your backpack.",
				m_session->GetPlayer()->GetName(),
				gold, silver, copper );
		}
	}
...
bool ChatHandler::HandleTriggerCommand(const char* args, WorldSession* m_session)
{
	...
	{
		SystemMessage(m_session, "Could not find trigger %s", (args == NULL ? "NULL" : args));
		return true;
	}
	...
	SystemMessage(m_session, "Teleported to trigger %u on [%u][%.2f][%.2f][%.2f]", pTrigger->AreaTriggerID,
...
bool ChatHandler::HandleUnlearnCommand(const char* args, WorldSession * m_session)
{
	...
	if(SpellId == 0)
	{
		SystemMessage(m_session, "You must specify a spell id.");
		return true;
	}
	...
	{
		SystemMessageToPlr(plr, "Removed spell %u.", SpellId);
		plr->removeSpell(SpellId, false, false, 0);
	}
	else
	{
		SystemMessage(m_session, "That player does not have spell %u learnt.", SpellId);
	}
...
bool ChatHandler::HandleNpcSpawnLinkCommand(const char* args, WorldSession *m_session)
{
	...
	if(valcount)
	{
		...
		SystemMessage(m_session, "Spawn linking for this NPC has been updated: %u", id);
	}
	else
	{
		SystemMessage(m_session, "Sql entry invalid %u", id);
	}
