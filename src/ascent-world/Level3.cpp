...
bool ChatHandler::HandleClearCooldownsCommand(const char *args, WorldSession *m_session)
{
	...
	if(plr->getClass()==WARRIOR)
	{
		...
		SystemMessage(m_session, "Cleared all Warrior cooldowns.");
		return true;
	}
	if(plr->getClass()==PALADIN)
	{
		...
		SystemMessage(m_session, "Cleared all Paladin cooldowns.");
		return true;
	}
	if(plr->getClass()==HUNTER)
	{
		...
		SystemMessage(m_session, "Cleared all Hunter cooldowns.");
		return true;
	}
	if(plr->getClass()==ROGUE)
	{
		...
		SystemMessage(m_session, "Cleared all Rogue cooldowns.");
		return true;
	}
	if(plr->getClass()==PRIEST)
	{
		...
		SystemMessage(m_session, "Cleared all Priest cooldowns.");
		return true;
	}
	if(plr->getClass()==SHAMAN)
	{
		...
		SystemMessage(m_session, "Cleared all Shaman cooldowns.");
		return true;
	}
	if(plr->getClass()==MAGE)
	{
		...
		SystemMessage(m_session, "Cleared all Mage cooldowns.");
		return true;
	}
	if(plr->getClass()==WARLOCK)
	{
		...
		SystemMessage(m_session, "Cleared all Warlock cooldowns.");
		return true;
	}
	if(plr->getClass()==DRUID)
	{
		...
		SystemMessage(m_session, "Cleared all Druid cooldowns.");
		return true;
	}
	return true;
}
...
bool ChatHandler::HandleLearnCommand(const char* args, WorldSession *m_session)
{
	...
	plr->addSpell(spell);
	SystemMessageToPlr(plr, "%s taught you Spell %d", m_session->GetPlayer()->GetName(), spell);
...
bool ChatHandler::HandleUnBanCharacterCommand(const char* args, WorldSession *m_session)
{
	...
	{
		SystemMessage(m_session, "A character name and reason is required.");
		return true;
	}
	...
	{
		SystemMessage(m_session, "Player %s not found ingame.", Character);
	}
...
bool ChatHandler::HandleNpcInfoCommand(const char *args, WorldSession *m_session)
{
	...
	if(crt->GetCreatureName())
		SystemMessage(m_session, "Showing creature info for %s", crt->GetCreatureName()->Name);
	...
	SystemMessage(m_session, "Base Health: %d", crt->GetUInt32Value(UNIT_FIELD_BASE_HEALTH));
	SystemMessage(m_session, "Base Armor: %d", crt->GetUInt32Value(UNIT_FIELD_RESISTANCES));
	SystemMessage(m_session, "Base Mana: %d", crt->GetUInt32Value(UNIT_FIELD_MAXPOWER1));
	SystemMessage(m_session, "Base Holy: %d", crt->GetUInt32Value(UNIT_FIELD_RESISTANCES_01));
	SystemMessage(m_session, "Base Fire: %d", crt->GetUInt32Value(UNIT_FIELD_RESISTANCES_02));
	SystemMessage(m_session, "Base Nature: %d", crt->GetUInt32Value(UNIT_FIELD_RESISTANCES_03));
	SystemMessage(m_session, "Base Frost: %d", crt->GetUInt32Value(UNIT_FIELD_RESISTANCES_04));
	SystemMessage(m_session, "Base Shadow: %d", crt->GetUInt32Value(UNIT_FIELD_RESISTANCES_05));
	SystemMessage(m_session, "Base Arcane: %d", crt->GetUInt32Value(UNIT_FIELD_RESISTANCES_06));
...
bool ChatHandler::HandleIncreaseWeaponSkill(const char *args, WorldSession *m_session)
{
	...
	if(!SubClassSkill)
	{
		SystemMessage(m_session, "Can't find skill ID :-/");
		return false;
	}

	uint32 skill = SubClassSkill;

	SystemMessage(m_session, "Modifying skill line %d. Advancing %d times.", skill, cnt);
...
bool ChatHandler::HandleResetTalentsCommand(const char* args, WorldSession *m_session)
{
	...
	SystemMessageToPlr(plr, "%s reset all your talents.", m_session->GetPlayer()->GetName());
...
bool ChatHandler::HandleResetSpellsCommand(const char* args, WorldSession *m_session)
{
	...
	SystemMessage(m_session, "%s reset all your spells to starting values.", m_session->GetPlayer()->GetName());
...
bool ChatHandler::HandleAccountLevelCommand(const char * args, WorldSession * m_session)
{
	...
	SystemMessage(m_session, "Account '%s' level has been updated to '%s'. The change will be effective immediately.", account, gmlevel);
...
bool ChatHandler::HandleAccountUnbanCommand(const char * args, WorldSession * m_session)
{
	...
	SystemMessage(m_session, "Account '%s' has been unbanned. This change will be effective immediately.", pAccount);
...
bool ChatHandler::HandleAccountBannedCommand(const char * args, WorldSession * m_session)
{
	...
	*pDuration = 0;
	...
	SystemMessage(m_session, "Account '%s' has been banned %s%s. The change will be effective immediately.", pAccount, 
...
bool ChatHandler::HandleAccountMuteCommand(const char * args, WorldSession * m_session)
{
	...
	*pDuration = 0;
	...
	SystemMessage(m_session, "Account '%s' has been muted until %s. The change will be effective immediately.", pAccount, 
		tsstr.c_str());
...
bool ChatHandler::HandleAccountUnmuteCommand(const char * args, WorldSession * m_session)
{
	...
	SystemMessage(m_session, "Account '%s' has been unmuted.", args);
...
bool ChatHandler::HandleRemoveAurasCommand(const char *args, WorldSession *m_session)
{
	...
	SystemMessage(m_session, "Removing all auras...");
...
bool ChatHandler::HandleRemoveRessurectionSickessAuraCommand(const char *args, WorldSession *m_session)
{
	...
	SystemMessage(m_session, "Removing ressurection sickness...");
...
bool ChatHandler::HandleParalyzeCommand(const char* args, WorldSession *m_session)
{
	...
	SystemMessage(m_session, "Rooting target.");
	SystemMessageToPlr( static_cast< Player* >( plr ), "You have been rooted by %s.", m_session->GetPlayer()->GetName() );
	WorldPacket data;
...
bool ChatHandler::HandleUnParalyzeCommand(const char* args, WorldSession *m_session)
{
	...
	SystemMessage(m_session, "Unrooting target.");
	SystemMessageToPlr( static_cast< Player* >( plr ), "You have been unrooted by %s.", m_session->GetPlayer()->GetName() );
	WorldPacket data;
...
bool ChatHandler::HandleSetMotdCommand(const char* args, WorldSession* m_session)
{
	...
	{
		SystemMessage(m_session, "You must specify a message.");
		return true;
	}

	SystemMessage(m_session, "Motd has been set to: %s", args);
...
bool ChatHandler::HandleAddItemSetCommand(const char* args, WorldSession* m_session)
{
	...
	if(!setid)
	{
		SystemMessage(m_session, "You must specify a setid.");
		return true;
	}

	Player *chr = getSelectedChar(m_session);
	if (chr == NULL) {
	SystemMessage(m_session, "Unable to select character.");
	return true;
	}
	...
	if(!entry || !l)
	{
		SystemMessage(m_session, "Invalid item set.");
		return true;
	}
	//const char* setname = sItemSetStore.LookupString(entry->name);
	SystemMessage(m_session, "Searching item set %u...", setid);
	...
	SystemMessage(m_session, "Added set to inventory complete. Time: %u ms", getMSTime() - start);
	return true;
}

bool ChatHandler::HandleExitInstanceCommand(const char* args, WorldSession* m_session)
{
	SystemMessage(m_session, "Attempting to exit from instance...");
	bool result = m_session->GetPlayer()->ExitInstance();
	if(!result)
	{
		SystemMessage(m_session, "Entry points not found.");
		return true;
	} else {
		SystemMessage(m_session, "Removal successful.");
		return true;
	}
}
...
bool ChatHandler::HandleCastTimeCheatCommand(const char* args, WorldSession* m_session)
{
	...
	SystemMessage(m_session, "%s cast time cheat on %s.", val ? "Deactivating" : "Activating", plyr->GetName());
	SystemMessageToPlr(plyr, "%s %s a cast time cheat on you.", m_session->GetPlayer()->GetName(), val ? "deactivated" : "activated");
...
bool ChatHandler::HandleCooldownCheatCommand(const char* args, WorldSession* m_session)
{
	...
	SystemMessage(m_session, "%s cooldown cheat on %s.", val ? "Deactivating" : "Activating", plyr->GetName());
	SystemMessageToPlr(plyr, "%s %s a cooldown cheat on you.", m_session->GetPlayer()->GetName(), val ? "deactivated" : "activated");
...
bool ChatHandler::HandleGodModeCommand(const char* args, WorldSession* m_session)
{
	...
	SystemMessage(m_session, "%s godmode cheat on %s.", val ? "Deactivating" : "Activating", plyr->GetName());
	SystemMessageToPlr(plyr, "%s %s a godmode cheat on you.", m_session->GetPlayer()->GetName(), val ? "deactivated" : "activated");
...
bool ChatHandler::HandlePowerCheatCommand(const char* args, WorldSession* m_session)
{
	...
	SystemMessage(m_session, "%s power cheat on %s.", val ? "Deactivating" : "Activating", plyr->GetName());
	SystemMessageToPlr(plyr, "%s %s a power cheat on you.", m_session->GetPlayer()->GetName(), val ? "deactivated" : "activated");
...
#define print_cheat_status(CheatName, CheatVariable) SystemMessage(m_session, "%s%s: %s%s", MSG_COLOR_LIGHTBLUE, CheatName, \
	...
	SystemMessage(m_session, "Showing cheat status for: %s", plyr->GetName());
...
bool ChatHandler::HandleFlyCommand(const char* args, WorldSession* m_session)
{
	...
	SystemMessage(chr->GetSession(), "Flying mode enabled.");
	return 1;
}
...
bool ChatHandler::HandleLandCommand(const char* args, WorldSession* m_session)
{
	...
	fly << uint32(5);
	chr->SendMessageToSet(&fly, true);
	SystemMessage(chr->GetSession(), "Flying mode disabled.");
	return 1;
}
...
bool ChatHandler::HandleFlySpeedCheatCommand(const char* args, WorldSession* m_session)
{
	...
	SystemMessage(m_session, "Setting the fly speed of %s to %f.", plr->GetName(), Speed);
	SystemMessage(plr->GetSession(), "%s set your fly speed to %f.", m_session->GetPlayer()->GetName(), Speed);
...
bool ChatHandler::HandleModifyLevelCommand(const char* args, WorldSession* m_session)
{
	...
	// Set level message
	SystemMessage(m_session, "Setting the level of %s to %u.", plr->GetName(), Level);
	SystemMessageToPlr(plr, "%s set your level to %u.", m_session->GetPlayer()->GetName(), Level);
...
/*	if(!args || strlen(args) < 2)
	...
	{
		SystemMessage(m_session, "Invalid creature spawn template: %u", Entry);
		return true;
	}
...
bool ChatHandler::HandlePetSpawnAIBot(const char* args, WorldSession *m_session)
{
	...
	{
		SystemMessage(m_session, "You need a total of %u coins to afford a bot", botprice);
		return false;
	}
	...
	if ( botType!=0 )
	{
		SystemMessage(m_session, "Incorrect value. Accepting value 0 only = healbot :)");
		return true;
	}
...
bool ChatHandler::HandleAddPetSpellCommand(const char* args, WorldSession* m_session)
{
	...
	if(pPet == 0)
	{
		SystemMessage(m_session, "You have no pet.");
		return true;
	}
	...
	if(!SpellId || !spell)
	{
		SystemMessage(m_session, "Invalid spell id requested.");
		return true;
	}

	pPet->AddSpell(spell, true);
	SystemMessage(m_session, "Added spell %u to your pet.", SpellId);
	return true;
}
...
bool ChatHandler::HandleRenamePetCommand(const char* args, WorldSession* m_session)
{
	...
	if(strlen(args) < 1)
	{
		SystemMessage(m_session, "You must specify a name.");
		return true;
	}

	SystemMessage(m_session, "Renamed your pet to %s.", args);
	pPet->Rename(args);
	return true;
}
...
bool ChatHandler::HandleAllowWhispersCommand(const char* args, WorldSession* m_session)
{
	...
	SystemMessage(m_session, "Now accepting whispers from %s.", plr->GetName());
	return true;
}
...
bool ChatHandler::HandleBlockWhispersCommand(const char* args, WorldSession* m_session)
{
	...
	SystemMessage(m_session, "Now blocking whispers from %s.", plr->GetName());
	return true;
}
...
bool ChatHandler::HandleAdvanceAllSkillsCommand(const char* args, WorldSession* m_session)
{
	...
	if(!amt)
	{
		SystemMessage(m_session, "An amount to increment is required.");
		return true;
	}
	...
	SystemMessageToPlr(plr, "Advanced all your skill lines by %u points.", amt);
...
bool ChatHandler::HandleKillByPlayerCommand(const char* args, WorldSession* m_session)
{
	if(!args || strlen(args) < 2)
	{
		SystemMessage(m_session, "A player's name is required.");
		return true;
	}
...
bool ChatHandler::HandleKillByIPCommand(const char* args, WorldSession* m_session)
{
	if(!args || strlen(args) < 2)
	{
		SystemMessage(m_session, "An IP is required.");
		return true;
	}
...
bool ChatHandler::HandleCastAllCommand(const char* args, WorldSession* m_session)
{
	if(!args || strlen(args) < 2)
	{
		SystemMessage(m_session, "No spellid specified.");
		return true;
	}
	Player * plr;
	uint32 spellid = atol(args);
	...
	if(!info)
	{
		SystemMessage(m_session, "Invalid spell specified.");
		return true;
	}
	...
	for (int i = 0; i < 3; i++)
	{
		if (info->Effect[i] == 36) //SPELL_EFFECT_LEARN_SPELL - 36
		{
			...
			SystemMessage(m_session, "Learn spell specified.");
			return true;
		}
	}
	...
	SystemMessage(m_session, "Casted spell %u on all players!", spellid);
	return true;
}
...
bool ChatHandler::HandleModPeriodCommand(const char* args, WorldSession * m_session)
{
	...
	if(trans == 0)
	{
		SystemMessage(m_session, "You must be on a transporter.");
		return true;
	}
	...
	if(np == 0)
	{
		SystemMessage(m_session, "A time in ms must be specified.");
		return true;
	}

	trans->SetPeriod(np);
	SystemMessage(m_session, "Period of %s set to %u.", trans->GetInfo()->Name, np);
	return true;
}
...
bool ChatHandler::HandleFormationLink1Command(const char* args, WorldSession * m_session)
{
	...
	SystemMessage(m_session, "Linkup \"master\" set to %s.", pCreature->GetCreatureName()->Name);
	return true;
}
...
bool ChatHandler::HandleFormationLink2Command(const char* args, WorldSession * m_session)
{
	...
	{
		SystemMessage(m_session, "You must specify a distance and angle.");
		return true;
	}

	if(m_session->GetPlayer()->linkTarget == 0 || m_session->GetPlayer()->linkTarget->GetTypeId() != TYPEID_UNIT)
	{
		SystemMessage(m_session, "Master not selected. select the master, and use formationlink1.");
		return true;
	}
	...
	SystemMessage(m_session, "%s linked up to %s with a distance of %f at %f radians.", slave->GetCreatureName()->Name, 
...
bool ChatHandler::HandleStackCheatCommand(const char* args, WorldSession * m_session)
{
	...
	SystemMessage(m_session, "%s aura stack cheat on %s.", val ? "Deactivating" : "Activating", plyr->GetName());
	SystemMessageToPlr(plyr, "%s %s an aura stack cheat on you.", m_session->GetPlayer()->GetName(), val ? "deactivated" : "activated");
...
bool ChatHandler::HandleTriggerpassCheatCommand(const char* args, WorldSession * m_session)
{
	...
	SystemMessage(m_session, "%s areatrigger prerequisites immunity cheat on %s.", val ? "Deactivated" : "Activated", plr->GetName());
	SystemMessageToPlr(plr, "%s %s areatrigger prerequisites immunity cheat on you.", m_session->GetPlayer()->GetName(), val ? "deactivated" : "activated");
...
bool ChatHandler::HandleResetSkillsCommand(const char* args, WorldSession * m_session)
{
	...
	SystemMessage(m_session, "Reset skills to default.");
...
bool ChatHandler::HandlePlayerInfo(const char* args, WorldSession * m_session)
{
	...
	if(strlen(args) >= 2) // char name can be 2 letters
	{
		...
		if(!plr)
		{
			SystemMessage(m_session, "Unable to locate player %s.", args);		
			return true;
		}
	}
	else
	...
	if(!plr->GetSession())
	{
		SystemMessage(m_session, "ERROR: this player hasn't got any session !");
		return true;
	}
	if(!plr->GetSession()->GetSocket())
	{
		SystemMessage(m_session, "ERROR: this player hasn't got any socket !");
		return true;
	}
...
//	char* infos = new char[128];
	...
	SystemMessage(m_session, "%s is a %s %s %s", plr->GetName(),
		(plr->getGender()?"Female":"Male"), races[plr->getRace()], classes[plr->getClass()]);

	SystemMessage(m_session, "%s has played %s at this level",(plr->getGender()?"She":"He"), playedLevel);
	SystemMessage(m_session, "and %s overall", playedTotal);

	SystemMessage(m_session, "%s is connecting from account '%s'[%u] with permissions '%s'",
		(plr->getGender()?"She":"He"), sess->GetAccountName().c_str(), sess->GetAccountId(), sess->GetPermissions());

	SystemMessage(m_session, "%s uses %s (build %u)", (plr->getGender()?"She":"He"),
		(sess->HasFlag(ACCOUNT_FLAG_XPACK_01)?"WoW Burning Crusade":"WoW"), sess->GetClientBuild());

	SystemMessage(m_session, "%s IP is '%s', and has a latency of %ums", (plr->getGender()?"Her":"His"),
...
bool ChatHandler::HandleGlobalPlaySoundCommand(const char* args, WorldSession * m_session)
{
	...
	SystemMessage(m_session, "Broadcasted sound %u to server.", sound);
...
bool ChatHandler::HandleIPBanCommand(const char * args, WorldSession * m_session)
{
	...
	*pDuration = 0;
	...
	{
		SystemMessage(m_session, "Invalid IPv4 address [%s]", pIp);
	...
	if (i == string::npos)
	{
		SystemMessage(m_session, "Lack of CIDR address assumes a 32bit match (if you don't understand, dont worry, it worked)");
		IP.append("/32");
...
bool ChatHandler::HandleCreatureSpawnCommand(const char *args, WorldSession *m_session)
{
	...
	if(proto == 0 || info == 0)
	{
		SystemMessage(m_session, "Invalid entry id.");
		return true;
	}
	...
	SystemMessage(m_session, "Spawned a creature `%s` with entry %u at %f %f %f on map %u", info->Name, 
...
bool ChatHandler::HandleRemoveItemCommand(const char * args, WorldSession * m_session)
{
	...
	SystemMessage(m_session, "Removing %u copies of item %u from %s's inventory.", ocount, item_id, plr->GetName());
	SystemMessage(plr->GetSession(), "%s removed %u copies of item %u from your inventory.", m_session->GetPlayer()->GetName(), ocount, item_id);
	return true;
}
...
bool ChatHandler::HandleForceRenameCommand(const char * args, WorldSession * m_session)
{
	...
	if(pi == 0)
	{
		SystemMessage(m_session, "Player with that name not found.");
		return true;
	}
	...
	else
	{
		...
		plr->SaveToDB(false);
		SystemMessageToPlr(plr, "%s forced your character to be renamed next logon.", m_session->GetPlayer()->GetName());
	}

	CharacterDatabase.Execute("INSERT INTO banned_names VALUES('%s')", CharacterDatabase.EscapeString(string(pi->name)).c_str());
	SystemMessage(m_session, "Forcing %s to rename his character next logon.", args);
...
bool ChatHandler::HandleSetStandingCommand(const char * args, WorldSession * m_session)
{
	...
	SystemMessage(m_session, "Setting standing of %u to %d on %s.", faction, standing, plr->GetName());
	plr->SetStanding(faction, standing);
	SystemMessageToPlr(plr, "%s set your standing of faction %u to %d.", m_session->GetPlayer()->GetName(), faction, standing);
...
bool ChatHandler::HandleLookupItemCommand(const char * args, WorldSession * m_session)
{
	...
	if(x.length() < 4)
	{
		SystemMessage(m_session, "Your search string must be at least 5 characters long.");
		return true;
	}

	StorageContainerIterator<ItemPrototype> * itr = ItemPrototypeStorage.MakeIterator();

	SystemMessage(m_session, "Starting search of item `%s`...", x.c_str());
	uint32 t = getMSTime();
	ItemPrototype * it;
	uint32 count = 0;
	while(!itr->AtEnd())
	{
		...
		if(FindXinYString(x, it->lowercase_name))
		{
			...
			if(count == 25)
			{
				SystemMessage(m_session, "More than 25 results returned. aborting.");
				break;
			}
		}
	...
	SystemMessage(m_session, "Search completed in %u ms.", getMSTime() - t);
	return true;
}
...
bool ChatHandler::HandleLookupCreatureCommand(const char * args, WorldSession * m_session)
{
	...
	SystemMessage(m_session, "Starting search of creature `%s`...", x.c_str());
	uint32 t = getMSTime();
	CreatureInfo * i;
	uint32 count = 0;
	while(!itr->AtEnd())
...
bool ChatHandler::HandleGORotate(const char * args, WorldSession * m_session)
{
	...
	if( !go )
	{
		SystemMessage(m_session, "No selected GameObject...");
		return true;
	}
...
bool ChatHandler::HandleNpcPossessCommand(const char * args, WorldSession * m_session)
{
	...
	if(!pTarget)
	{
		SystemMessage(m_session, "You must select a player/creature.");
		return true;
	}

	m_session->GetPlayer()->Possess(pTarget);
	SystemMessage(m_session, "Possessed "I64FMT, pTarget->GetGUID());
	return true;
}
...
bool ChatHandler::HandleNpcUnPossessCommand(const char * args, WorldSession * m_session)
{
	...
	SystemMessage(m_session, "Removed any possessed targets.");
	return true;
}
...
bool ChatHandler::HandleAIAgentDebugBegin(const char * args, WorldSession * m_session)
{
	...
	SystemMessage(m_session, "Loaded %u spells for testing.", aiagent_spells.size());
	return true;
}
...
bool ChatHandler::HandleAIAgentDebugContinue(const char * args, WorldSession * m_session)
{
	...
	for(uint32 i = 0; i < count; ++i)
	{
		...
		SystemMessage(m_session, "Casting %u, "MSG_COLOR_SUBWHITE"%u remaining.", sp->Id, aiagent_spells.size());
	...
	if(!aiagent_spells.size())
		SystemMessage(m_session, "Finished.");
	/*else
		SystemMessage(m_session, "Got %u remaining.", aiagent_spells.size());*/
	return true;
}
...
bool ChatHandler::HandleAIAgentDebugSkip(const char * args, WorldSession * m_session)
{
	...
	SystemMessage(m_session, "Erased %u spells.", count);
	return true;
}
...
bool ChatHandler::HandleDispelAllCommand(const char * args, WorldSession * m_session)
{
	...
	SystemMessage(m_session, "Dispel action done.");
	return true;
}
...
bool ChatHandler::HandleAddTrainerSpellCommand( const char * args, WorldSession * m_session )
{
	...
	if( pTrainer == NULL )
	{
		SystemMessage(m_session, "Target is not a trainer.");
		return true;
	}
	...
	if(pSpell==NULL)
	{
		SystemMessage(m_session, "Invalid spell.");
		return true;
	}
	...
	{
		SystemMessage(m_session, "No. You're not doing that.");
		return true;
	}
...
bool ChatHandler::HandleAddInvItemCommand(const char *args, WorldSession *m_session)
{
	...
	if(it)
	{
		...
		SystemMessage(m_session, "Item %d is not a valid item!",itemid);
		return true;
	}
}
